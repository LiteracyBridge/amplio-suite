<template>
  <div class="w-full">
    <div class="flex my-2">
      ...
      <div class="mx-2 mt-2 text-gray-600"
      ...
      <div>
        <p class="font-light text-2xl text-gray-800">Deployment {{ deployment.deploymentnumber }} </p>
      </div>
      <v-input
        aria-label="`Deployment ${name}`"
        placeholder="Deployment Name"
        :class="(!name)||(name.length===0)?'invalid border-red-500 border-2 rounded':''"
        type="text"
        :name="`Deployment ${name}`"
        mx="-mt-1 mx-2"
        :value="name"
        @change="setDeploymentName({ deployment: deployment, deploymentname: $event.target.value })"
      />
      ...
      <v-input
        ...
        @change="setDeploymentStartdate({ deployment: deployment, startdate: $event.target.value })"
        mx="mx-0"
      />
      <v-input
        class="-mt-1 mx-2"
        ...
        @change="setDeploymentEnddate({ deployment: deployment, enddate: $event.target.value })"
        mx="mx-0"
      />

      <button v-if="expanded" class="btn" @click="onAddPlaylist()" :disabled="!canAddPlaylist">
        Add Playlist
      </button>
...
export default {
  ...
  computed: {
    ...
    canAddPlaylist() {
      ...
      let canAdd = (this.deployment.playlists.length === 0 ||
    ...
    canRemoveDeployment: {
      get() {
        ...
        // delete icon.
        return this.canRemove && this.deployment.playlists.length === 0 && !this.deployment.deployed;
      }
    },
